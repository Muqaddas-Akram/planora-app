...
const TripsScreen = ({ navigation }) => {
  ...
  // Categorize trips into active, upcoming, and recent (completed within 7 days)
  const categorizeTripsByDate = useCallback((tripsList) => {
    ...
    tripsList.forEach(trip => {
      ...
      if (tripStartDate <= today && tripEndDate >= today) {
        // Trip is currently active (started on or before today, ends on or after today)
        active.push(trip);
      } else if (tripStartDate > today) {
        // Trip is upcoming (starts after today)
        upcoming.push(trip);
      } else if (tripEndDate > sevenDaysAgo && tripEndDate < today) {
        // Trip ended within last 7 days
        recent.push({
          ...trip,
          daysUntilRemoval: Math.ceil((tripEndDate.getTime() + 7 * 24 * 60 * 60 * 1000 - today.getTime()) / (24 * 60 * 60 * 1000))
        });
      }
    });
  ...
  const loadTrips = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      // Categorize trips
      const { active, upcoming, recent } = categorizeTripsByDate(tripsList);
    ...
    <View style={styles.recentTripCard}>
      <View style={styles.recentTripHeader}>
        ...
        <View style={styles.removalWarning}>
          <Text style={styles.removalText}>Removing in {item.daysUntilRemoval} days</Text>
        </View>
      </View>
      ...
      <View style={styles.recentTripStats}>
        <View style={styles.statItem}>
          ...
          <Text style={styles.statValue}>{item.currency} {item.totalExpenses.toFixed(0)}</Text>
        </View>
        <View style={styles.statItem}>
          <Text style={styles.statLabel}>Budget</Text>
          <Text style={styles.statValue}>{item.currency} {item.budget.toFixed(0)}</Text>
        </View>
        <View style={styles.statItem}>
      ...
      {item.expenses && item.expenses.length > 0 && (
        <View style={styles.expensesList}>
          <Text style={styles.expensesTitle}>Expenses ({item.expenses.length})</Text>
          {item.expenses.slice(0, 3).map((expense, idx) => (
            <View key={expense.id} style={styles.expenseItem}>
              ...
              <Text style={styles.expenseAmount}>{item.currency} {expense.amount.toFixed(0)}</Text>
            </View>
          ))}
          {item.expenses.length > 3 && (
            <Text style={styles.moreExpenses}>+{item.expenses.length - 3} more expenses</Text>
          )}
        </View>
      )}

      <Text style={styles.dataRemovalNotice}>
        ⏱️ This data will be automatically removed after 7 days
      </Text>
    </View>
  );
  ...
  const sections = [
    ...
    {
      title: 'Recent Trips (Last 7 Days)',
      ...
      <View style={styles.content}>
        {loading ? (
          ...
          <SectionList
            ...
            renderItem={({ item, section }) => {
              if (item.isEmpty) {
                ...
                if (section.type === 'upcoming') return renderEmptyUpcomingTrips();
                return null;
              }
          ...
          <View style={styles.emptyContainer}>
            <Ionicons name="briefcase-outline" size={80} color={COLORS.secondary} />
